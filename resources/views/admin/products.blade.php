...
@section('title', __('dashboard/product.title-all'))
...
<div class="main-dashboard-panels">
    ...
    <div class="main-dashboard-panel">
        ...
        <h2 class="font-semibold">{{ __('dashboard/product.title-all') }}</h2>
                    ...
                    <div class="products__info">
                        <div class="products__left-side">
                            ...
                            <div class="product__left-side__content">
                                ...
                                <a href="{{ route('admin.product', ['product' => $product['slug']]) }}">
                                    <h3 class="title">{{ Str::limit($product['name'], 40, ' ...') }}</h3>
                                </a>
                                <span class="offers">{{ __('dashboard/product.offers_all_length') }}: {{ count($product->offers) }}</span><br/>
                                <span class="offers">{{ __('dashboard/product.offers_unhandled_length') }}: {{ count($product->offersUnhandled) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="products__right-side">
                            ...
                            <div class="actions">
                                <a href="{{ route('admin.product', ['product' => $product['slug']]) }}" class="action-btn pr-3"><i
                                        class="fa fa-pencil" aria-hidden="true"></i></a>
                                <form method="post"
                                    action="{{ route('admin.product', ['product' => $product['slug']]) }}"
